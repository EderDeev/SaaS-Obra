            ...
            DB::table($table)
                ...
                ->chunkById(200, function ($records) use ($table, $add): void {
                    foreach ($records as $record) {
                        ...
                        if ($add) {
                            $canAnalyze = in_array(OrdemServicoPermissions::ANALYZE, $permissions, true);
                            $canApprove = in_array(OrdemServicoPermissions::APPROVE, $permissions, true);

                            if (($canAnalyze || $canApprove) && ! in_array(OrdemServicoPermissions::METRICS, $permissions, true)) {
                                $permissions[] = OrdemServicoPermissions::METRICS;
                            }
                        } else {
                            $permissions = array_values(array_filter(
                                $permissions,
                                fn ($permission): bool => $permission !== OrdemServicoPermissions::METRICS
                            ));
                        }
